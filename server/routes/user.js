...
router.put(
  ...
  [
    check("address.mobile", "Mobile number should contains 10 digits").isLength(
      { min: 10, max: 12 }
    ),
  ],
...
// @access User (Public)
router.put(
  "/user/checkout/cartItems/:userId",
  isSignedin,
  isAuthenticated,
  checkOutCartItems
);
...
// @desc Get User By ID
// @access ADMIN
router.get("/admin/user/:adminId/:usrId", isSignedin, isAdmin, getUserByAdmin);
